...
function get_fraction_cmds(selected, is_manage)
    local cmds = {
		{cmd = 'time', description = 'Посмотреть время',  text = '/me взглянул{sex} на свои часы с гравировкой YouHelper by Bulat_Nabiev Team&/time&/do На часах видно время {get_time}.', arg = '', enable = true, waiting = '2', bind = "{}"},
...
imgui.OnFrame(
    ...
    function(player)
		...
		if MODULE.Initial.step == 0 then
			...
			if imgui.IsItemClicked() then openLink('https://youhelper.com/terms') end
