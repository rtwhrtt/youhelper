...
script_name("YouHelper")
script_description('YouHelper v2.5 | Arizona RP | t.me/youhelper_rp2')
script_author("Низаев Булат / Bulat_Nabiev")
script_version("0.1")
----------------------------------------------- INIT ---------------------------------------------
...
------------------------------------------ INIT CRASH INFO ---------------------------------------
local errors_handler_path = worked_dir .. '/.YouHelper Handler.lua'
if not doesFileExist(errors_handler_path) then
	local helper_prefix = '/.YouHelper '
	local content = [[
...
function onSystemMessage(msg, type, script)
	if script and script.name == 'YouHelper' and msg and ((msg:find('stack traceback')) or (type == 3 and not msg:find('Script died due to an error'))) then
		local errorMessage = ('{ffffff}Произошла непредусмотренная ошибка в работе скрипта, из-за чего он был отключён!\n\n' ..
		'Отправьте скриншот этого диалога в {ff9900}тех.поддержку YouHelper (GitHub: github.com/rtwhrtt/youhelper){ffffff}.\n\n' ..
		'Детали возникшей ошибки:\n{ff6666}' .. msg .. '{ffffff}\n\n' ..
		'Полный лог работы скрипта: \n{66BB6A}' .. getLog())
		print("[YouHelper] error logged") --
...
-------------------------------------------- JSON SETTINGS ---------------------------------------
local config_dir = worked_dir .. '/YouHelper'
local settings = {}
...
load_settings()
do
    if sizeX and sizeX>0 and settings and settings.windows_pos then
        local function cp(v,mn,mx) return math.max(mn,math.min(mx,v or mn)) end
        local wp=settings.windows_pos
        local defs={patrool_menu={x=sizeX/2,y=sizeY/2},post_menu={x=sizeX/2,y=sizeY/2},
            wanteds_menu={x=sizeX/2,y=sizeY/2},zeks_menu={x=sizeX*0.83,y=sizeY/2},
            mobile_fastmenu_button={x=sizeX*0.12,y=sizeY*0.43},pie={x=sizeX*0.7,y=sizeY*0.7}}
        for k,def in pairs(defs) do
            if not wp[k] then wp[k]={x=def.x,y=def.y}
            else
                local px=tonumber(wp[k].x) or def.x;local py=tonumber(wp[k].y) or def.y
                if px<10 or px>sizeX-50 or py<10 or py>sizeY-30 then px=def.x;py=def.y end
                wp[k].x=cp(px,10,sizeX-100);wp[k].y=cp(py,10,sizeY-50)
            end
        end
    end
end
...
if hotkey_no_errors then
	...
	function createHotkeyForCommand(command)
		...
		if command.arg == "" and command.bind ~= nil and command.bind ~= '{}' and command.bind ~= '[]' then
			...
			sampAddChatMessage('[YouHelper] {ffffff}Создан хоткей для команды ' .. message_color_hex .. '/' .. command.cmd .. ' {ffffff}на клавишу '  .. message_color_hex .. getNameKeysFrom(command.bind), message_color)
		end
	end
...
function handleNewWeapon(weaponId)
    sampAddChatMessage('[YouHelper] {ffffff}Обнаружено новое оружие с ID ' .. message_color_hex .. weaponId .. '{ffffff}. Ему автоматически назначено имя "оружие" и расположение "спина"', message_color)
    sampAddChatMessage('[YouHelper] {ffffff}Изменить название или расположение оружия можно через команду /rpguns', message_color)
...
function processWeaponChange(oldGun, nowGun)
    ...
    if not modules.rpgun.data.gunActions.off[oldGun] or not modules.rpgun.data.gunActions.on[nowGun] then
        sampAddChatMessage('[YouHelper | Ассистент] {ffffff}Инициализация оружия...', message_color)
		initialize_guns()
		return
    end
...
function main()
	...
	if settings.general.fraction_mode == '' then
		...
		repeat wait(200) until settings.general.fraction_mode ~= '' and not MODULE.Initial.Window[0]
	end
	...
	if settings.general.analytics then sendAnalytics() end
		-- Player menu by E key
		if not sampIsChatInputActive() and not sampIsDialogActive() and
		   not MODULE.Main.Window[0] and isKeyJustPressed(0x45) then
			local myX, myY, myZ = getCharCoordinates(PLAYER_PED)
			local closest_id, closest_dist = -1, 8
			...
			if closest_id >= 0 then
				MODULE.PlayerMenu.player_id = closest_id
				MODULE.PlayerMenu.Window[0] = true
			end
		end
	...
	while true do
			...
			if MODULE.Patrool.active then
				...
				if settings.mj.auto_change_code_siren and isCharInAnyCar(PLAYER_PED) then
					...
					if currentSirenState ~= lastSirenState then
						...
						sampAddChatMessage("[YouHelper | Ассистент] {ffffff}Проблесковые маячки " .. (currentSirenState and "активированы (CODE 3)." or "деактивированы (CODE 4)."), message_color)
		...
		if MODULE.CruiseControl.wait_point then
			...
			if bool then
				...
				sampAddChatMessage('[YouHelper] {ffffff}Координаты места назначения успешно получены!', message_color)
				...
				lua_thread.create(function()
					...
					MODULE.CruiseControl.active = true
						settings.general.mcruise_enable = true; save_settings()
					wait(2000)
		...
		if MODULE.CruiseControl.active then
			local function stop()
				MODULE.CruiseControl.active = false
						settings.general.mcruise_enable = false; save_settings()
			...
			if not isCharInAnyCar(PLAYER_PED) then
				sampAddChatMessage('[YouHelper] {ffffff}Вы должны находится в транспортном средстве!', message_color)
				stop()
			elseif not isCarEngineOn(storeCarCharIsInNoSave(PLAYER_PED)) then
				sampAddChatMessage('[YouHelper] {ffffff}Двигатель вашего транспортного средства заглох!', message_color)
				stop()
			elseif locateCharInCar2d(PLAYER_PED, MODULE.CruiseControl.point.x, MODULE.CruiseControl.point.y, 15, 15, false) then
				...
				taskCarDriveToCoord(PLAYER_PED, storeCarCharIsInNoSave(PLAYER_PED), MODULE.CruiseControl.point.x, MODULE.CruiseControl.point.y, MODULE.CruiseControl.point.z, settings.general.mcruise_speed / 3.6, 0, 0, 2)
			end
		end
...
function load_modules()
	...
	if settings.general.piemenu then
		if pie_no_errors then 
			...
			sampAddChatMessage('[YouHelper] {ffffff}Модуль PieMenu отключён: отсутствует необходимая библиотека!', message_color)
...
function welcome_message()
	if not sampIsLocalPlayerSpawned() then 
		sampAddChatMessage('[YouHelper] {ffffff}Для завершения загрузки хелпера войдите на сервер.', message_color)
		repeat wait(0) until sampIsLocalPlayerSpawned()
	end

	sampAddChatMessage('[YouHelper] {ffffff}Загрузка хелпера успешно завершена!', message_color)
	show_notify('info', 'YouHelper', "Загрузка хелпера успешно завершена!", 3000)
	print('Полная загрузка хелпера успешно завершена!')

	if hotkey_no_errors and settings.general.bind_mainmenu then	
		sampAddChatMessage('[YouHelper] {ffffff}Для открытия меню хелпера нажмите ' .. message_color_hex .. getNameKeysFrom(settings.general.bind_mainmenu) .. ' {ffffff}или используйте команду ' .. message_color_hex .. '/helper', message_color)
	else
		sampAddChatMessage('[YouHelper] {ffffff}Для открытия меню хелпера используйте команду ' .. message_color_hex .. '/helper', message_color)
	end
		...
		if not MODULE.Binder.state.isActive then
			...
			if cmd_arg == '{arg}' then
				if args and args ~= '' then
					...
					sampAddChatMessage('[YouHelper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [любое значение]', message_color)
					play_sound()
				end
			elseif cmd_arg == '{id}' then
				if isParamSampID(args) then
					...
					sampAddChatMessage('[YouHelper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока]', message_color)
					play_sound()
				end
			elseif cmd_arg == '{id} {arg}' then
				if args and args ~= '' then
					...
					if isParamSampID(id) and arg then
						...
						sampAddChatMessage('[YouHelper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока] [любое значение]', message_color)
						play_sound()
					end
				else
					sampAddChatMessage('[YouHelper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока] [любое значение]', message_color)
            ...
            elseif cmd_arg == '{id} {number} {arg}' then
				if args and args ~= '' then
					...
					if isParamSampID(id) and number and arg then
						...
						sampAddChatMessage('[YouHelper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока] [любое число] [любое значение]', message_color)
						play_sound()
					end
				else
					sampAddChatMessage('[YouHelper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока] [любое число] [любое значение]', message_color)
				...
				lua_thread.create(function()
					...
					for line_index, line in ipairs(lines) do
						if MODULE.Binder.state.isStop then 
							...
							sampAddChatMessage('[YouHelper] {ffffff}Отыгровка команды /' .. chat_cmd .. " успешно остановлена!", message_color) 
							break
						elseif line == "{pause}" then
							sampAddChatMessage('[YouHelper] {ffffff}Команда /' .. chat_cmd .. ' поставлена на паузу!', message_color)
							if not IS_MOBILE then
								if hotkey_no_errors and settings.general.bind_action then
									sampAddChatMessage('[YouHelper] {ffffff}Для продолжения нажмите ' .. message_color_hex .. getNameKeysFrom(settings.general.bind_action) .. ' {ffffff}или вызовите курсор открыв чат (T/F6)', message_color)
								else
									sampAddChatMessage('[YouHelper] {ffffff}Для продолжения вызовите курсор открыв чат (T/F6)', message_color)
								end
							end
							...
							if not MODULE.Binder.state.isStop then
								sampAddChatMessage('[YouHelper] {ffffff}Продолжаю отыгровку команды /' .. chat_cmd, message_color)	
							end	
			...
			sampAddChatMessage('[YouHelper] {ffffff}Дождитесь завершения предыдущей команды.', message_color)
			play_sound()
		end
	end)
end
function info_stop_command()
	if IS_MOBILE and settings.general.mobile_stop_button then
		sampAddChatMessage('[YouHelper] {ffffff}Для остановки отыгровки используйте команду ' .. message_color_hex .. '/stop {ffffff}или кнопку в нижней части экрана.', message_color)
		MODULE.CommandStop.Window[0] = true
	elseif hotkey_no_errors and settings.general.bind_command_stop then
		sampAddChatMessage('[YouHelper] {ffffff}Для остановки отыгровки используйте команду ' .. message_color_hex .. '/stop {ffffff}или нажмите ' .. message_color_hex .. getNameKeysFrom(settings.general.bind_command_stop) .. '{ffffff}.', message_color)
	else
		sampAddChatMessage('[YouHelper] {ffffff}Для остановки отыгровки используйте команду ' .. message_color_hex .. '/stop{ffffff}.', message_color)
	end
end
function find_and_use_command(cmd, cmd_arg)
	...
	sampAddChatMessage('[YouHelper] {ffffff}Не удалось найти бинд этой команды! Попробуйте сбросить настройки хелпера.', message_color)
	play_sound()
end
function initialize_commands()
				sampRegisterChatCommand("seladala", function()
		lua_thread.create(function()
			sampSendChat("Села = дала")
			wait(1500)
			sampSendChat("Я покажу тебе, что такое love is")
			wait(1500)
			sampSendChat("Ты танцуешь, как Дженнифер Лопес")
			wait(1500)
			sampSendChat("Твои большие таланты в сторис")
			wait(1500)
			sampSendChat("Я смотрю их, я твой фанат")
			wait(1500)
			sampSendChat("Твои глаза — красивый карий")
			wait(1500)
			sampSendChat("Я пишу тебе комментарии")
			wait(1500)
			sampSendChat("Чё там? Кездесейiк, вино iшпедiк па?")
			wait(1500)
			sampSendChat("Меня зовут Канат")
			wait(1500)
			sampSendChat("Канат, ты же женат")
			wait(1500)
			sampSendChat("Трое детей, ёб вашу мать")
			wait(1500)
			sampSendChat("Уйде отырмайсын ба?")
			wait(1500)
			sampSendChat("Кетiн кысып отырмайсын ба?")
			wait(1500)
			sampSendChat("Не канает так, Канат")
			wait(1500)
			sampSendChat("Жена узнает всё, Канат")
			wait(1500)
			sampSendChat("Я всё ей расскажу")
			wait(1500)
			sampSendChat("Э, маган ше? С любовью, твой Канат")
			wait(1500)
		end)
	end)
	...
	sampRegisterChatCommand("binder", function()
		...
		sampAddChatMessage('[YouHelper] {ffffff}Биндер находится во вкладке "Команды и RP отыгровки" - "RP команды".', message_color)
	end)
	...
	sampRegisterChatCommand("stop", function()
		if MODULE.Binder.state.isActive then 
			...
			sampAddChatMessage('[YouHelper] {ffffff}В данный момент нет активных команд или RP-отыгровок.', message_color)
		end
	end)
	sampRegisterChatCommand("fixsize", function()
		...
		save_settings()
		sampAddChatMessage('[YouHelper] {ffffff}Размер интерфейса хелпера сброшен до стандартного значения. Выполняю перезапуск...', message_color)
		reload_script = true
		thisScript():reload()
	end)
	sampRegisterChatCommand("rpguns", function()
		if settings.general.rp_guns then
			...
			sampAddChatMessage('[YouHelper] {ffffff}Включите функцию "RP отыгровка оружия" в разделе ' .. message_color_hex .. '/helper - Функции ' .. modules.player.data.fraction_tag, message_color)
		end
	end)
	...
	sampRegisterChatCommand("mcruise", function() MODULE.MCruise.Window[0] = not MODULE.MCruise.Window[0] end)
		...
		if server == 0 or server < 200 then
			if not MODULE.Binder.state.isActive then
				if MODULE.CruiseControl.active then
					...
					sampAddChatMessage('[YouHelper] {ffffff}Режим "CRUISE CONTROL" отключен!', message_color)
				else
					if not isCharInAnyCar(PLAYER_PED) then
						sampAddChatMessage('[YouHelper] {ffffff}Вы должны находится в транспортном средстве!', message_color)
						return
					...
					if not (isCarEngineOn(car)) then
						sampAddChatMessage('[YouHelper] {ffffff}Заведите двигатель вашего транспортного средства!', message_color)
						return
					end
					local driver = getDriverOfCar(car)
					if driver ~= PLAYER_PED then
						sampAddChatMessage('[YouHelper] {ffffff}Вы должны быть водителем транспортного средства!', message_color)
						return
					end
					local bool, x, y, z = getTargetBlipCoordinates()
					if bool then
						sampAddChatMessage('[YouHelper] {ffffff}Удалите свою старую метку с карты!', message_color)
						return
					end
					MODULE.CruiseControl.point = {x = 0, y = 0, z = 0}
					MODULE.CruiseControl.wait_point = true
					sampAddChatMessage('[YouHelper] {ffffff}Выберите пункт назнанения, поставив метку на карте', message_color)
				end
			else
				sampAddChatMessage('[YouHelper] {ffffff}Дождитесь завершения предыдущей команды.', message_color)
				play_sound()
			...
			sampAddChatMessage('[YouHelper] {ffffff}Данная функция поддерживается только на карте GTA San Andreas. Карты CRMP и Vice City не поддерживаются.', message_color)
			play_sound()
		end
	end)
	sampRegisterChatCommand("activate", function()
		sampAddChatMessage('[YouHelper] {ffffff}К сожалению нельзя прямо из игры перейти на VIP версию!', message_color) 
		sampAddChatMessage('[YouHelper] {ffffff}подробнее: github.com/rtwhrtt/youhelper', message_color)
		sampAddChatMessage('[YouHelper] {ffffff}После активации ключика, в боте используйте команду /helper для получения VIP', message_color)
		-- sampAddChatMessage('[YouHelper] {ffffff}У вас уже установлена VIP версия, и вам доступны все функции!', message_color)
	end)
	sampRegisterChatCommand("debug", function()
		MODULE.DEBUG = not MODULE.DEBUG 
		sampAddChatMessage('[YouHelper] {ffffff}Отслеживание серверных данных ' .. (MODULE.DEBUG and 'включено.' or 'выключено.'), message_color)
	end)
		...
		sampRegisterChatCommand("mb", function(arg)
			if not MODULE.Binder.state.isActive then
				if MODULE.Members.Window[0] then
					...
					sampAddChatMessage('[YouHelper] {ffffff}Меню списка сотрудников закрыто!', message_color)
				else
					MODULE.Members.new = {} 
		...
		sampRegisterChatCommand("sob", function(arg)
			if not MODULE.Binder.state.isActive then
				if isParamSampID(arg) then
					...
					sampAddChatMessage('[YouHelper] {ffffff}Используйте ' .. message_color_hex .. '/sob [ID игрока]', message_color)
					play_sound()
				end	
			else
				sampAddChatMessage('[YouHelper] {ffffff}Дождитесь завершения предыдущей команды.', message_color)
	...
	if isMode('police') or isMode('fbi') then
			...
			if not MODULE.Binder.state.isActive then
				if isParamSampID(arg) then
					if #modules.smart_uk.data ~= 0 then
						...
						sampAddChatMessage('[YouHelper] {ffffff}Сначала загрузите/заполните систему умного розыска в ' .. message_color_hex .. '/helper - Функции ' .. modules.player.data.fraction_tag, message_color)
						play_sound()
					end
				else
					sampAddChatMessage('[YouHelper] {ffffff}Используйте ' .. message_color_hex .. '/sum [ID игрока]', message_color)
					play_sound()
				end	
			else
				sampAddChatMessage('[YouHelper] {ffffff}Дождитесь завершения предыдущей команды.', message_color)
			...
			if not MODULE.Binder.state.isActive then
				if isParamSampID(arg) then
					if #modules.smart_pdd.data ~= 0 then
						...
						sampAddChatMessage('[YouHelper] {ffffff}Сначала загрузите/заполните систему умных штрафов в ' .. message_color_hex .. '/helper - Функции ' .. modules.player.data.fraction_tag, message_color)
						play_sound()
					end
				else
					sampAddChatMessage('[YouHelper] {ffffff}Используйте ' .. message_color_hex .. '/tsm [ID игрока]', message_color)
					play_sound()
				end
			else
				sampAddChatMessage('[YouHelper] {ffffff}Дождитесь завершения предыдущей команды.', message_color)
		...
		sampRegisterChatCommand("wanted", function(arg)
			...
			sampAddChatMessage('[YouHelper] {ffffff}Рекомендуется использовать команду ' .. message_color_hex .. '/wanteds {ffffff}для автоматического сканирования всего списка розыска.', message_color)
		end)
		sampRegisterChatCommand("wanteds", function(arg)
			if MODULE.Wanted.Window[0] or MODULE.Wanted.updwanteds.stop then
				...
				sampAddChatMessage('[YouHelper] {ffffff}Меню списка преступников закрыто!', message_color)
			elseif not MODULE.Wanted.checker then
				lua_thread.create(function()
					local max_lvl = isMode('fbi') and 7 or 6
					sampAddChatMessage('[YouHelper] {ffffff}Сканирование /wanted, ожидайте ' .. message_color_hex .. max_lvl .. ' {ffffff}секунд...', message_color)
					show_notify('info', 'YouHelper', "Сканирование /wanted...", 2500)
					MODULE.Wanted.new = {}
					...
					if #MODULE.Wanted.new == 0 then
						sampAddChatMessage('[YouHelper] {ffffff}Сейчас на сервере нету игроков в розыске!', message_color)
					else
						sampAddChatMessage('[YouHelper] {ffffff}Сканирование списка /wanted окончено! Найдено преступников: ' .. message_color_hex .. #MODULE.Wanted.new, message_color)
				...
				sampAddChatMessage('[YouHelper] {ffffff}Дождитесь завершения сканирования!', message_color)
				play_sound()
			end
		end)
			...
			if not MODULE.Binder.state.isActive then
				if isParamSampID(arg) then
					if #modules.smart_rptp.data ~= 0 then
						...
						sampAddChatMessage('[YouHelper] {ffffff}Сначала загрузите/заполните систему умного срока в ' .. message_color_hex .. '/helper - Функции ' .. modules.player.data.fraction_tag, message_color)
						play_sound()
					end
				else
					sampAddChatMessage('[YouHelper] {ffffff}Используйте ' .. message_color_hex .. '/pum [ID игрока]', message_color)
					play_sound()
				end	
			else
				sampAddChatMessage('[YouHelper] {ffffff}Дождитесь завершения предыдущей команды.', message_color)
		...
		sampRegisterChatCommand('zeks', function()
			if settings.gov.custom_zeks then
				if MODULE.Zeks.Window[0] or MODULE.Zeks.updzeks.stop then
					...
					sampAddChatMessage('[YouHelper] {ffffff}Меню списка заключенных закрыто!', message_color)
				elseif not MODULE.Zeks.checker then
					sampAddChatMessage('[YouHelper] {ffffff}Сканирование /zeks...', message_color)
					show_notify('info', 'YouHelper', "Сканирование /zeks...", 2500)
					MODULE.Zeks.new = {}
				...
				sampAddChatMessage('[YouHelper] {ffffff}Вы можете включить кастомное меню /zeks с авто-обновлением в ' .. message_color_hex .. '/helper - Функции Право', message_color)
				sampSendChat('/zeks')
			end
		end)
	end
			...
			if not MODULE.Binder.state.isActive then
				lua_thread.create(function()
					...
					if MODULE.Binder.state.isStop then 
						...
						sampAddChatMessage('[YouHelper] {ffffff}Отыгровка команды /spcar успешно остановлена!', message_color) 
						return
					end
...
function show_fast_menu(id)
	if isParamSampID(id) then 
		...
		if hotkey_no_errors and settings.general.bind_fastmenu then
			sampAddChatMessage('[YouHelper] {ffffff}Используйте ' .. message_color_hex .. '/hm [ID игрока] {ffffff}или наведитесь на игрока через ' .. message_color_hex .. 'ПКМ + ' .. getNameKeysFrom(settings.general.bind_fastmenu), message_color) 
		else
			sampAddChatMessage('[YouHelper] {ffffff}Используйте ' .. message_color_hex .. '/hm [ID игрока]', message_color)
		end 
		play_sound()
	end 
end
function show_leader_fast_menu(id)
	if isParamSampID(id) then
		...
		if hotkey_no_errors and settings.general.bind_leader_fastmenu then
			sampAddChatMessage('[YouHelper] {ffffff}Используйте ' .. message_color_hex .. '/lm [ID игрока] {ffffff}или наведитесь на игрока через ' .. message_color_hex .. 'ПКМ + ' .. getNameKeysFrom(settings.general.bind_leader_fastmenu), message_color) 
		else
			sampAddChatMessage('[YouHelper] {ffffff}Используйте ' .. message_color_hex .. '/lm [ID игрока]', message_color)
		end 
		play_sound()
	end
end
...
function get_vehicle_name(id)
	...
	sampAddChatMessage('[YouHelper] {ffffff}Не удалось получить модель т/c ' .. id .. " ID, обновляю конфиг транспорта...", message_color)
	download_file = 'arz_veh'
	downloadFileFromUrlToPath('https://raw.githubusercontent.com/rtwhrtt/youhelper/main/SmartVEH/Vehicles' .. 
...
function downloadFileFromUrlToPath(url, path)
	local function on_finish_download()
		if download_file == 'helper' then
			sampAddChatMessage('[YouHelper] {ffffff}Загрузка новой версии хелпера успешно завершена! Перезагрузка..',  message_color)
			reload_script = true
			thisScript():reload()
		elseif download_file == 'smart_uk' then
			sampAddChatMessage('[YouHelper] {ffffff}Загрузка системы умной выдачи розыска для сервера ' .. message_color_hex .. getServerName(getServerNumber()) .. ' [' .. getServerNumber() ..  '] {ffffff}завершена успешно!',  message_color)
			sampAddChatMessage('[YouHelper] {ffffff}Теперь вы можете использовать команду ' .. message_color_hex .. '/sum [ID игрока]', message_color)
			MODULE.Main.Window[0] = false
			play_sound()
			load_module('smart_uk')
		elseif download_file == 'smart_pdd' then
			sampAddChatMessage('[YouHelper] {ffffff}Загрузка системы умной выдачи штрафов для сервера ' .. message_color_hex .. getServerName(getServerNumber()) .. ' [' .. getServerNumber() ..  '] {ffffff}завершена успешно!',  message_color)
			sampAddChatMessage('[YouHelper] {ffffff}Теперь вы можете использовать команду ' .. message_color_hex .. '/tsm [ID игрока]', message_color)
			MODULE.Main.Window[0] = false
			play_sound()
			load_module('smart_pdd')
		elseif download_file == 'smart_rptp' then
			sampAddChatMessage('[YouHelper] {ffffff}Загрузка системы умного срока для сервера ' .. message_color_hex .. getServerName(getServerNumber()) .. ' [' .. getServerNumber() ..  '] {ffffff}завершена успешно!',  message_color)
			sampAddChatMessage('[YouHelper] {ffffff}Теперь вы можете использовать команду ' .. message_color_hex .. '/pum [ID игрока]', message_color)
			MODULE.Main.Window[0] = false
			play_sound()
			load_module('smart_rptp')
		elseif download_file == 'arz_veh' then
			sampAddChatMessage('[YouHelper] {ffffff}Загрузка всех кастомных т/с успешно заверешена!',  message_color)
			play_sound()
			load_module('arz_veh')
			cache_vehicles()
	...
	if IS_MOBILE then
		...
		if ok then
			...
			sampAddChatMessage("[YouHelper] {ffffff}Ошибка загрузки файла: " .. tostring(err), message_color)
		end
	else
...
function check_update() isUpdateChecked = true end
...
function check_resources()
	...
	if not doesFileExist(config_dir .. '/Resourse/logo.png') then
		...
		downloadFileFromUrlToPath('https://raw.githubusercontent.com/rtwhrtt/youhelper/main/foto3.png', config_dir .. '/Resourse/logo.png')
	end
	if not doesFileExist(config_dir .. "/Resourse/notify.mp3") then
		print('Подгружаю звук оповещений хелпера...')
		downloadFileFromUrlToPath('https://raw.githubusercontent.com/rtwhrtt/youhelper/main/Resourse/notify.mp3', config_dir .. "/Resourse/notify.mp3")
	end
...
function get_fraction_cmds(selected, is_manage)
    local cmds = {
		{cmd = 'time', description = 'Посмотреть время',  text = '/me посмотрел{sex} на часы с гравировкой YouHelper&/time&/do На часах видно время {get_time}.', arg = '', enable = true, waiting = '2', bind = "{}"},
...
function import_data_from_old_helpers()	
	...
	local function import_settings(folder)
		...
		sampAddChatMessage('[YouHelper] {ffffff}Импорт ваших команд (биндов) и заметок из ' .. message_color_hex .. folder .. '{ffffff} успешно завершен!', message_color)
	...
	local function import_split(folder)
		...
		if doesFileExist(notesPath) then 
			...
			if n and n.note then
				...
				sampAddChatMessage('[YouHelper] {ffffff}Импорт ваших заметок из ' .. message_color_hex .. folder .. ' Helper {ffffff} успешно завершен!', message_color)
				os.remove(notesPath)
			end
		end
		...
		if doesFileExist(cmdsPath) then 
			...
			if c then
				...
				sampAddChatMessage('[YouHelper] {ffffff}Импорт ваших команд (биндов) из ' .. message_color_hex .. folder .. ' Helper {ffffff} успешно завершен!', message_color)
				os.remove(cmdsPath)
			end
		end
	end
	for _, helpers in ipairs({"Mafia", "FD", "Prison", "GOV", "Government", "YouHelper"}) do
	...
	local function safeMove(folder, file, target)
		...
		if readJsonSafe(p) then 
			os.rename(p, target)
			sampAddChatMessage('[YouHelper] {ffffff}Импорт "' .. file .. '" из ' .. message_color_hex .. folder .. '{ffffff} успешно завершен!', message_color)
		end
	end
	safeMove("SMI Helper", "Ads.json", modules.ads_history.path)
	safeMove("YouHelper", "SmartUK.json", modules.smart_uk.path)
	safeMove("YouHelper", "SmartPDD.json", modules.smart_pdd.path)
...
function delete_old_helpers()
    ...
    local correct_path = worked_dir .. "/YouHelper.lua"
	if current_path ~= correct_path then
		sampAddChatMessage('[YouHelper] {ffffff}Исправляю название файла хелпера для корректной работы обновлений...', message_color)
        if doesFileExist(correct_path) then os.remove(correct_path) end
        os.rename(current_path, correct_path)
    end

	local old_helpers = {"YouHelper", "Hospital", "SMI", "AS", "FD", "GOV", "Government", "Mafia", "Prison"}
...
function delete_helper_data(checker)
	...
	if checker then
		...
		sampAddChatMessage('[YouHelper] {ffffff}Хелпер полностью удалён из вашего устройства!', message_color)
		reload_script = true
		thisScript():unload()
	else
		sampAddChatMessage('[YouHelper] {ffffff}Перезагрузка хелпера...', message_color)
		reload_script = true
		thisScript():reload()
	end
end
if isMode('police') or isMode('fbi') then
	function form_su(name, playerID, message)
		...
		if (modules.player.data.fraction_rank_number >= rank) then
			...
			sampAddChatMessage('[YouHelper | Ассистент] {ffffff}Используйте ' .. message_color_hex .. '/givefsu ' .. playerID .. '{ffffff} чтобы выдать розыск по запросу офицера ' .. message_color_hex .. name, message_color)
			play_sound()
		else
			sampAddChatMessage('[YouHelper | Ассистент] {ffffff}Для выдачи розыска по запросу нужно иметь ' .. rank .. '-й ранг, но вы только ' .. modules.player.data.fraction_rank_number .. '-й ранг :(', message_color)
		end
	end
end
		...
		if (nick and id and message and tonumber(id) ~= select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))) then
			local function check_end_time()
				lua_thread.create(function()
					...
					if MODULE.HealChat.bool then
						...
						MODULE.HealChat.bool = false
						sampAddChatMessage('[YouHelper] {ffffff}Вы не успели вылечить игрока ' .. sampGetPlayerNickname(id), message_color)
					end
				end)
			end
			for hello_bro, keyword in ipairs(MODULE.HealChat.worlds) do
				if (message:rupper():find(keyword:rupper())) then
					if IS_MOBILE then
						sampAddChatMessage('[YouHelper] {ffffff}Чтоб вылечить игрока ' .. sampGetPlayerNickname(id) .. ', в течении 5-ти секунд нажмите кнопку', message_color)
						MODULE.HealChat.player_id = id
						MODULE.HealChat.bool = true
						MODULE.HealChat.Window[0] = true
						check_end_time()
					elseif hotkey_no_errors then
						sampAddChatMessage('[YouHelper] {ffffff}Чтобы вылечить игрока ' .. sampGetPlayerNickname(id) .. ' нажмите ' .. message_color_hex .. getNameKeysFrom(settings.general.bind_action) .. ' {ffffff}в течении 5-ти секунд!', message_color)
						show_notify('info', 'YouHelper', 'Нажмите ' .. getNameKeysFrom(settings.general.bind_action) .. ' чтобы быстро вылечить игрока', 5000)
						MODULE.HealChat.player_id = id
						MODULE.HealChat.bool = true
						check_end_time()
					end
...
if isMode('smi') then
	function try_send_ad(text)
		if text == '' then
			sampAddChatMessage('[YouHelper] {ffffff}Нельзя отправить пустое обьявление!', message_color)
			play_sound()
			return false
		end
		...
		if MODULE.SmiEdit.ad_repeat_count >= 51 then
			sampAddChatMessage('[YouHelper] {ffffff}Не удалось отправить обьяву, у вас слишком много спец.символов (цифры/точки/кавычки)!', message_color)
			play_sound()
			...
			if modules.ads_history.data then
				...
				sampAddChatMessage('[YouHelper] {ffffff}Сломался файл ' .. modules.ads_history.path, message_color)
				sampAddChatMessage('[YouHelper] {ffffff}Удалите его, либо если шарите, то найдите ошибку и исправьте (файл в кодировке 1251)', message_color)
				play_sound()
			end
			return false
		end
...
function sampev.onShowTextDraw(id, data)
	...
	if data.text:find('~n~~n~~n~~n~~n~~n~~n~~n~~w~Style: ~r~Sport!') then
		sampAddChatMessage('[YouHelper] {ffffff}Активирован режим езды Sport!', message_color)
		return false
	end
	if data.text:find('~n~~n~~n~~n~~n~~n~~n~~n~~w~Style: ~g~Comfort!') then
		sampAddChatMessage('[YouHelper] {ffffff}Активирован режим езды Comfort!', message_color)
		return false
	end
end
...
function sampev.onSendTakeDamage(playerId,damage,weapon)
	...
	if playerId ~= 65535 then
		...
		if isParamSampID(playerId) and playerId1 ~= playerId2 and tonumber(playerId) ~= 0 and weapon then
			...
			if weapon_name then
				sampAddChatMessage('[YouHelper] {ffffff}Игрок ' .. sampGetPlayerNickname(playerId) .. '[' .. playerId .. '] напал на вас используя ' .. weapon_name .. '['.. weapon .. ']!', message_color)
				if isMode('police') or isMode('fbi') or isMode('army') or isMode('prison') then
					if ((MODULE.Patrool.Window[0]) and (MODULE.Patrool.ComboCode[0] ~= 1)) then
						sampAddChatMessage('[YouHelper | Ассистент] {ffffff}Ваш ситуационный код изменён на CODE 0.', message_color)
...
function sampev.onSendGiveDamage(playerId, damage, weapon, bodypart)
	...
	if playerId ~= 65535 then
		if (sampGetPlayerNickname(playerId) == 'Bogdan_Martelli' and getServerNumber() == '20') or sampGetPlayerNickname(playerId):find('%[20%]Bogdan_Martelli') then
			sampAddChatMessage('[YouHelper] {ffffff}Bogdan_Martelli - это разработчик YouHelper!', message_color)
			sampAddChatMessage('[YouHelper] {ffffff}Не нужно наносить урон разработчику хелпера, АСТАНАВИТЕСЬ :sob: :sob: :sob:', message_color)
			play_sound()
		end
	end
end
function sampev.onServerMessage(color, text)
	...
	if settings.general.ping and MODULE.Binder.tag.my_nick() and MODULE.Binder.tag.my_nick() ~= '' and text:find('@' .. MODULE.Binder.tag.my_nick(), 1, true) then
		sampAddChatMessage('[YouHelper] {ffffff}Кто-то упомянул вас в чате!', message_color)
		play_sound()
	end
	...
	if modules.player.data.fraction_rank_number >= 9 then
		if settings.general.auto_uninvite then
			...
			elseif text:find("(.+) заглушил%(а%) игрока (.+) на 1 минут. Причина: ПСЖ") and MODULE.LeadTools.checker then
				...
				if Name == MyName then
					sampAddChatMessage('[YouHelper | Ассистент] {ffffff}Увольняю игрока ' .. sampGetPlayerNickname(MODULE.LeadTools.auto_uninvite.player_id) .. '!', message_color)
					MODULE.LeadTools.checker = false
					find_and_use_command("/uninvite {id} {arg}", (MODULE.LeadTools.auto_uninvite.player_id .. ' ПСЖ'))
				else
					sampAddChatMessage('[YouHelper | Ассистент] {ffffff}Другой заместитель/лидер уже увольняет игрока ' .. sampGetPlayerNickname(MODULE.LeadTools.auto_uninvite.player_id) .. '!', message_color)
					MODULE.LeadTools.checker = false
				end
			end
		end
	...
	if settings.general.auto_accept_docs and text:find('^%[Новое предложение%].+offer') then
		sampAddChatMessage('[YouHelper | Ассистент] {ffffff}Открываю список предложений от игрока...', message_color)
		sampSendChat('/offer')
	end
	...
	if isMode('smi') then
		...
		or text:find('^VIP объявление:') or text:find('^Стандартное объявление:') then -- rodina
			...
			sampAddChatMessage('[YouHelper] {ffffff}Поступило новое обьявление от игрока ' .. message_color_hex .. nick, message_color)
			return false
		end
		if (text:find('^%[Ошибка%] %{ffffff%}Это объявление уже редактирует (.+).')) then
			local nick = text:match('редактирует (.+).')
			sampAddChatMessage('[YouHelper] {ffffff}Это обьявление уже редактирует игрок ' .. message_color_hex  .. nick, message_color)
			return false
		end
	...
	if (isMode('police') or isMode('fbi')) then
		...
		if ((MODULE.Patrool.active) and (text:find('^На этом автомобиле уже установлена маркировка.'))) then
			sampAddChatMessage('[YouHelper] {ffffff}Меняю макрировку в транспорте...', message_color)
		...
		if (text:find('^%[Информация%] {ffffff}Вы подобрали обломок, теперь вам нужно отнести его и {ff0000}положить в общую кучу')) then
			sampAddChatMessage('[YouHelper] {ffffff}Вы подобрали завал, теперь вам нужно отнести его в общую кучу!', message_color)
			return false
		end
		if (text:find('^%[Информация%] {ffffff}Вы положили обломок в общую кучу, отправляйтесь к следующему завалу.')) then
			sampAddChatMessage('[YouHelper] {ffffff}Вы положили завал в общую кучу, теперь отправляйтесь к следующему завалу.', message_color)
			return false
		end
	...
	if isMode('hospital') then
		...
		if text:find('^%(%( Чтобы принять вызов, введите /godeath (%d+). Оплата за вызов (.+) %)%)') then
			...
			if MODULE.GoDeath.locate == 'неизвестном' then
				sampAddChatMessage('[YouHelper] {ffffff}Из города ' .. message_color_hex .. MODULE.GoDeath.city .. ' {ffffff}поступил вызов о пострадавшем ' .. message_color_hex .. sampGetPlayerNickname(MODULE.GoDeath.player_id), message_color)
			else
				sampAddChatMessage('[YouHelper] {ffffff}Из города ' .. message_color_hex .. MODULE.GoDeath.city .. ' (' .. MODULE.GoDeath.locate .. ') {ffffff}поступил вызов о пострадавшем ' .. message_color_hex .. sampGetPlayerNickname(MODULE.GoDeath.player_id), message_color)
			end
			sampAddChatMessage('[YouHelper] {ffffff}Вылечив его вы получите ' .. price_godeath .. '! Чтобы принять вызов, используйте команду ' .. message_color_hex .. cmd .. ' ' .. MODULE.GoDeath.player_id, message_color)
			return false
		end
		if text:find("^Пациент (.+) вызывает врачей .+холл.+этаж") then
			sampAddChatMessage('[YouHelper] {ffffff}Пациент ' .. text:match("Пациент (.+) вызывает") .. ' вызывает врача в холл больницы!', message_color)
			return false
		end
	...
	if isMode('lc') then
		if text:find('^Вы отремонтировали дорожный знак: (.+) Ваша зарплата%: (.+)') then
			...
			sampAddChatMessage('[YouHelper] {ffffff}За ремонт дорожного знака вы заработали ' .. money, message_color)
			if AS_REMONT_DEBUG then
				sampAddChatMessage('[YouHelper | Ассистент] {ffffff}Запомнил все ваши действия ремонта знака, и готов их повторять!', message_color)
		...
		if text:find('^Вы установили дорожный знак: (.+) Ваша зарплата%: (.+)') then
			local money = text:match('Ваша зарплата%: (.+)')
			sampAddChatMessage('[YouHelper] {ffffff}За установку дорожного знака вы заработали ' .. money, message_color)
			if AS_INSTALL_DEBUG then
				sampAddChatMessage('[YouHelper | Ассистент] {ffffff}Запомнил все ваши действия установки знака, и готов их повторять!', message_color)
		...
		if text:find('^Вы взяли инструменты для ремонта дорожного знака.') then
			sampAddChatMessage('[YouHelper] {ffffff}Вы взяли инструменты для ремонта дорожного знака.', message_color)
			return false
		end
		if text:find('^%[Ошибка%](.+)У игрока уже есть такая лицензия сроком более чем (.+)') then
			local days = text:match('сроком более чем (.+)')
			sampAddChatMessage('[YouHelper] {ffffff}У игрока уже есть такая лицензия сроком более чем ' .. days, message_color)
		...
		if (text:find('^%[Ошибка%](.+)Вы не можете продавать лицензии на такой срок')) then
			sampAddChatMessage('[YouHelper] {ffffff}Ваш ранг ниже, чем требуется для выдачи данной лицензии!', message_color)
	...
	if isMode('fd') then
		if (text:find("Происшествие(.+)В штате произошел пожар! Ранг опасности (%d) звезды")) then
			...
			sampAddChatMessage('[YouHelper] {ffffff}В штате новый пожар ' .. MODULE.Fires.lvl .. ' степени опасности!', message_color)
			if (tonumber(MODULE.Fires.lvl) >= 2) then
				sampAddChatMessage('[YouHelper] {ffffff}Действует повышенная выплата за устранение пожара из-за высокого уровня опасности.', message_color)
			end
			sampSendChat('/fires')
			return false
		end
		if (text:find("%[Информация%] {ffffff}Вы прибыли на место пожара")) then
			...
			sampAddChatMessage('[YouHelper] {ffffff}Вы прибыли на место пожара.', message_color)
		...
		if (text:find("%[Информация%] {ffffff}Пожарная машина будет зареспавнена через (%d+) минут")) then
			sampAddChatMessage('[YouHelper] {ffffff}Пожарная машина будет зареспавнена через ' .. text:match("через (%d+) минут") .. ' минут!', message_color)
			return false
		end
		if (MODULE.Fires.isZone) then
			if text:find("%[Информация%] {......}Происшествие №(%d+)%: Все очаги возгорания ликвидированы") then
				sampAddChatMessage('[YouHelper] {ffffff}Все очаги возгорания ликвидированы!', message_color)
			...
			if text:find("%[Информация%] {ffffff}Отнесите пострадавшего в палатку.") then
				sampAddChatMessage('[YouHelper] {ffffff}Отнесите пострадавшего в палатку.', message_color)
			...
			if text:find("%[Информация%] {ffffff}Отлично! Вы спасли пострадавшего!") then
				sampAddChatMessage('[YouHelper] {ffffff}Вы спасли пострадавшего!', message_color)
			...
			if text:find("%[Информация%] {ffffff}Вы заработали на происшествие {90EE90}$(.+){FFFFFF}, забрать вознаграждение можно на базе организации") then
				...
				sampAddChatMessage('[YouHelper] {ffffff}Пожар устранён, за его ликвидацию вы заработали: ' .. message_color_hex .. '$' .. (text:match('{90EE90}$(.+){FFFFFF}') or 'nil'), message_color)
		...
		if (text:find("%[Информация%] {ffffff}Палатка возвращена Вам в инвентарь.")) then
			sampAddChatMessage('[YouHelper] {ffffff}Палатка возвращена вам в инвентарь.', message_color)
	...
	if isMode('ins') then
		if (text:find('^(.+) подал заявление на страхование имущества.') and color == -1048826369) then
			...
			sampAddChatMessage('[YouHelper] {ffffff}Игрок ' .. nick .. ' подал заявление на страхование имущества!', message_color)
		...
		if (text:find('^Вы заполнили вторую часть документов.')) then
			sampAddChatMessage('[YouHelper | Ассистент] {ffffff}Прохождение мини игры успешно завершено!', message_color)
			return false
		end
	end
	...
	if (text:find("^%[Подсказка%] {......}Номера телефонов государственных служб:")) then
		sampAddChatMessage('[YouHelper] {ffffff}Номера телефонов государственных служб:', message_color)
		sampAddChatMessage('[YouHelper] {ffffff}111 Баланс | 60 Время | 911 МЮ | 912 МЗ | 913 Такси | 914 Мехи | 8828 Банк | 997 Дома', message_color)
		return false
	end
	...
	if (text:find('Bogdan_Martelli') and getServerNumber() == '20') or text:find('%[20%]Bogdan_Martelli') then
		...
		if text:find('%[20%]Bogdan_Martelli%[%d+%]') then
			...
			text = string.gsub(text, '%[20%]Bogdan_Martelli%[%d+%]', message_color_hex .. '[20]rtwhrtt[' .. id .. ']' .. lastColor)
		elseif text:find('%[20%]Bogdan_Martelli') then
			text = string.gsub(text, '%[20%]Bogdan_Martelli', message_color_hex .. '[20]rtwhrtt' .. lastColor)
		elseif text:find('Bogdan_Martelli%[%d+%]') then
			local id = text:match('Bogdan_Martelli%[(%d+)%]') or ''
			text = string.gsub(text, 'Bogdan_Martelli%[%d+%]', message_color_hex .. 'rtwhrtt[' .. id .. ']' .. lastColor)
		elseif text:find('Bogdan_Martelli') then
			text = string.gsub(text, 'Bogdan_Martelli', message_color_hex .. 'rtwhrtt' .. lastColor)
		end
		return {color,text}
	end
end
...
function sampev.onSendCommand(text)
	if settings and settings.general and settings.general.rp_show_docs then
		local _cn, _ca = text:match("^/(%w+)%s+(%d+)")
		if _ca and sampIsPlayerConnected(tonumber(_ca)) then
			local _id = tonumber(_ca)
			if _cn == "showpass" then rp_spass_action(_id); return false
			elseif _cn == "showlic" then rp_slic_action(_id); return false
			elseif _cn == "showmc" then rp_smc_action(_id); return false
			elseif _cn == "showskill" then rp_sskill_action(_id); return false
			end
		end
	end
	if MODULE.DEBUG then
	...
	if isMode('smi') and MODULE.SmiEdit.is_active_ad and text:find('^%/newsredak') then
		sampAddChatMessage('[YouHelper | Ассистент] {ffffff}Дождитесь отправки предыдущего обьявления!', message_color)
		play_sound()
		return false
	end
	if settings.general.rp_chat then
...
function sampev.onShowDialog(dialogid, style, title, button1, button2, text)
	...
	if check_stats and (title:find('Основная статистика') or title:find('Статистика игрока')) then
		if text:find("Имя") then
			modules.player.data.name_surname = text:match("{ffffff}Имя %(\xf0\xf3\xf1%.%):%s+{......}([^\n\r]+)") or translate(text:match("{......}Имя: {......}([^%[\n\r]+)%s*%[")) or translate(modules.player.data.nick)

			sampAddChatMessage('[YouHelper] {ffffff}Ваше имя и фамилия обнаружены: ' .. (modules.player.data.name_surname or ''), message_color)
        end
		if text:find("Пол:") then
			modules.player.data.sex = text:match("{FFFFFF}Пол: {......}%[(.-)]") or text:match("{ffffff}Пол:%s+{......}([^\n\r]+)")
			sampAddChatMessage('[YouHelper] {ffffff}Ваш пол обнаружен: ' .. modules.player.data.sex, message_color)
		end
		if text:find("Организация:") then
			...
			if data then
				sampAddChatMessage('[YouHelper] {ffffff}Ваша организация обнаружена, это: '..modules.player.data.fraction, message_color)
				modules.player.data.fraction_tag = data[1]
				settings.general.fraction_mode = data[2]
				sampAddChatMessage('[YouHelper] {ffffff}Вашей организации присвоен тег '..modules.player.data.fraction_tag .. ". Но вы можете изменить его.", message_color)
				if text:find("Должность:") then
					...
					sampAddChatMessage('[YouHelper] {ffffff}Ваша должность обнаружена, это: ' .. modules.player.data.fraction_rank .. " (" .. modules.player.data.fraction_rank_number .. ")", message_color)
					if modules.player.data.fraction_rank_number >= 9 then
				...
				sampAddChatMessage('[YouHelper] {ffffff}Не удалось получить вашу организацию и должность!', message_color)
				sampAddChatMessage('[YouHelper] {ffffff}Присвоил вам режим без организации (ЖДЛС - Бомж - 1).', message_color)
				sampAddChatMessage('[YouHelper] {ffffff}Если вы действительно состоите в организации - перенастройте хелпер вручную.', message_color)
			end
			if old_fraction_mode ~= '' and old_fraction_mode ~= 'none' and old_fraction_mode ~= settings.general.fraction_mode then
				sampAddChatMessage('[YouHelper] {ffffff}Вы теперь в другой фракции, поэтому удаляю команды ' .. old_fraction_mode:rupper(), message_color)
		...
		if check_stats_close then
			check_stats_close = false
			return false
		end
	end
	...
	if ((MODULE.Members.info.check) and (title:find('(.+)%(В сети: (%d+)%)') or title:find('В сети всего .+ чле.+организации'))) then
		...
		elseif #MODULE.Members.new ~= 0 then
			...
			if not settings.general.auto_update_members then
				sampAddChatMessage('[YouHelper] {ffffff}Вы можете включить авто-обновление списка /mb /helper - Функции ' .. modules.player.data.fraction_tag .. '!', message_color)
			end
			MODULE.Members.Window[0] = true
		else
			sampSendDialogResponse(dialogid, 0, 0, 0)
			sampAddChatMessage('[YouHelper]{ffffff} Список сотрудников пуст!', message_color)
			MODULE.Members.info.check = false
        end
        return false
    end
		...
		if (MODULE.LeadTools.cleaner.uninvite) then
			...
			if title:find('Увольнение %(оффлайн%)') then
				...
				if #MODULE.LeadTools.cleaner.players_to_kick > 0 then
					sampAddChatMessage('[YouHelper] {ffffff} Найдено ' .. #MODULE.LeadTools.cleaner.players_to_kick .. ' игроков которые ' .. MODULE.LeadTools.cleaner.day_afk .. " дней не в сети!", message_color)
					lua_thread.create(function()
					...
					sampAddChatMessage('[YouHelper] {ffffff} Нету игроков которые ' .. MODULE.LeadTools.cleaner.day_afk .. " дней не в сети!",  message_color)
				end
	...
	if isMode('gov') then
		if settings.gov.anti_trivoga and (text:find('Вы действительно хотите вызвать сотрудников полиции?') or text:find('Вы действительно хотите {FFA11C}вызвать{FFFFFF} полицию?')) then
			sampAddChatMessage('[YouHelper] {ffffff}Тревожная кнопка отключена. Для включения используйте /helper - Функции Право', message_color)
			sampSendDialogResponse(dialogid, 2, 0, 0)
			return false
		end
		if MODULE.Zeks.checker and title:find("нные под стражу") then
			...
			if #MODULE.Zeks.new == 0 then
				sampAddChatMessage('[YouHelper] {ffffff}Сейчас на сервере нету заключенных игроков!', message_color)
			else
				sampAddChatMessage('[YouHelper] {ffffff}Сканирование /zeks окончено! Найдено заключенных игроков: ' .. message_color_hex .. #MODULE.Zeks.new, message_color)
	...
	if settings.general.auto_accept_docs then
		if (title:find('Активные предложения', 1, true) and (text:find('паспорт', 1, true) or text:find('лицензии', 1, true) or text:find('мед', 1, true))) then
			...
			elseif text:find('Принять предложение') then
				...
				sampAddChatMessage('[YouHelper | Ассистент] {ffffff}Запускаю отыгровку проверки документов игрока...', message_color)
				MODULE.Binder.state.isActive = true
	...
	if (isMode('hospital')) then
		if text:find("Проверьте и подтвердите данные перед выдачей мед карты") or text:find('Вы собираетесь предложить купить медкарту') then
			sampAddChatMessage('[YouHelper] {ffffff}Ожидайте пока игрок подтвердит получение мед. карты', message_color)
			sampSendDialogResponse(dialogid, 1, 0, 0)
			return false
		end
		-- rodina
	...
	if isMode('smi') then
		...
		if title:find('Редакция') or title:find('Выберите об.явление%:') then
			if text:find('На данный момент сообщений нет') then
				sampSendDialogResponse(dialogid, 1, 0, 0)
				sampAddChatMessage('[YouHelper] {ffffff}На данный момент нету обьявлений для редактирования!', message_color)
				return false
			end
		end 
	...
	if (isMode('lc')) then
		if title:find("Дорожные знаки") and (title:find("Los Santos") or title:find("San Fierro") or title:find("Las Venturas") or title:find("Lav Venturas")) and settings.lc.auto_find_clorest_znak then
			-- за основу взято https://github.com/rtwhrtt/youhelper by безликий
			local count = 0
			local znaks = {}
			for line in text:gmatch('[^\r\n]+') do
				count = count + 1
			...
			if not nearest then
				sampAddChatMessage("[YouHelper | Ассистент] {ffffff}В данном городе все дорожные знаки в норме!", message_color)
				sampSendDialogResponse(dialogid, 0, 0, "")
			else
				sampAddChatMessage("[YouHelper | Ассистент] {ffffff}Ближайший к вам знак " .. message_color_hex .. "№" .. nearest.number .. " {ffffff}(дистанция " .. message_color_hex .. nearest.distance .. "м{ffffff}, статус " .. message_color_hex .. nearest.status .. "{ffffff})", message_color)
	...
	if isMode('fd') then
		if title:find('Список происшествий') then
			if text:find('В данный момент все спокойно') then
				sampAddChatMessage('[YouHelper] {ffffff}В данный момент пожаров нету, можете отдыхать', message_color)
				sampSendDialogResponse(dialogid, 1, 0, 0)
				return false
			else
	...
	if isMode('ins') then
		if settings.ins.anti_trivoga and (text:find('Вы действительно хотите вызвать сотрудников полиции?') or text:find('Вы действительно хотите {FFA11C}вызвать{FFFFFF} полицию?')) then
			sampAddChatMessage('[YouHelper] {ffffff}Тревожная кнопка отключена. Для включения используйте /helper - Функции СТК', message_color)
			sampSendDialogResponse(dialogid, 2, 0, 0)
			return false
		end
		...
		if title:find('Заявки на страхование') then
			if text:find('На данный момент нет заявок на страхование') then
				sampAddChatMessage('[YouHelper] {ffffff}На данный момент нет заявок на страхование!', message_color)
				sampSendDialogResponse(dialogid, 1, 0, 0)
				return false
			end
		end
...
function sampev.onCreate3DText(id, color, position, distance, testLOS, attachedPlayerId, attachedVehicleId, text_3d)
	if text_3d and ((isMode('gov') and settings.gov.anti_trivoga) or (isMode())) then
		if text_3d:find('Тревожная кнопка') or text_3d:find('Кнопка для вызова полиции') then
			sampAddChatMessage('[YouHelper | Ассистент] {ffffff}Тревожная кнопка удалена из интерьера, поскольку вы отключили её.', message_color)
			return false
		end	
	end
end
function sampev.onPlayerChatBubble(playerid, color, distance, duration, message)
	...
	if (isMode('police') or isMode('fbi') or isMode('prison')) and settings.mj.anti_screpki then
		if message:find("(.+) достал скрепки для взлома наручников") then
			...
			sampAddChatMessage('[YouHelper] {ffffff}Внимание! ' .. nick .. '[' .. id .. '] использует скрепки и начинает взламывать наручники!', message_color)
			play_sound()
			...
			if result then
				...
				if getDistanceBetweenCoords3d(mx, my, mz, x, y, z) <= 1.5 then
					sampAddChatMessage('[YouHelper] {ffffff}Пытаюсь изьять скрепки у этого игрока...', message_color)
					find_and_use_command('/bot {id}', id)
				else
					sampAddChatMessage('[YouHelper] {ffffff}Подойдите к игроку ' .. nick .. ' и используйте команду /bot ' .. id, message_color)
				end
			...
			if cmd == 17 then
				...
				if (cmd:find('findGame') and cmd:find(' документов","Найдите ')) then
					if ((not isMode('ins')) or (isMode('ins') and settings.ins.hint_in_sort)) then
						...
						sampAddChatMessage("[YouHelper | Ассистент] {ffffff}Правильные конверты: " .. result .. ". Считать их нужно слева направо", message_color)
						sampShowDialog(897124, 'YouHelper - Ассистент', "Правильные конверты: " .. result .. ".\nСчитать их нужно слева направо", '{009EFF}Закрыть', '', 0)
					end
				end
			end
		end
		...
		if MODULE.Initial.step == 0 then
			if doesFileExist(config_dir .. '/Resourse/logo.png') then
				...
				if imgui.BeginChild('##init1_1', imgui.ImVec2(520 * settings.general.custom_dpi, 150 * settings.general.custom_dpi), true) then
					...
					imgui.CenterUnderlineText("https://github.com/rtwhrtt/youhelper")
					if imgui.IsItemClicked() then openLink('https://github.com/rtwhrtt/youhelper') end
					imgui.EndChild()
				end
			end
			...
			if imgui.IsItemHovered() then
				imgui.BeginTooltip()
				imgui.Text('https://youhelper.com/terms')
				imgui.EndTooltip()
			end
			if imgui.IsItemClicked() then openLink('https://github.com/rtwhrtt/youhelper/tree/main/terms') end
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(getHelperIcon() .. " YouHelper " .. getHelperIcon() .. "##main", MODULE.Main.Window, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize )
		change_dpi()
		if imgui.BeginTabBar(u8'Привет!') then	
			if imgui.BeginTabItem(fa.HOUSE..u8' Главное меню') then
				...
				if imgui.BeginChild('##2', imgui.ImVec2(589 * settings.general.custom_dpi, 169 * settings.general.custom_dpi), true) then
					...
					imgui.NextColumn()
					imgui.CenterColumnText(u8(modules.player.data.name_surname or ""))
					...
					if imgui.CenterColumnSmallButton(fa.PEN_TO_SQUARE .. '##name_surname') then
						...
						imgui.StrCopy(MODULE.Main.input, u8(modules.player.data.name_surname or ""))
						imgui.StrCopy(MODULE.Initial.input, u8(modules.player.data.nick or ""))
					...
					if imgui.CenterColumnSmallButton(fa.PEN_TO_SQUARE .. '##accent') then
						imgui.StrCopy(MODULE.Main.input, u8(modules.player.data.accent or ""))
					...
					imgui.NextColumn()
					imgui.CenterColumnText(u8(modules.player.data.sex or ""))
					imgui.NextColumn()
					...
					imgui.NextColumn()
					imgui.CenterColumnText(u8(modules.player.data.fraction or ""))
					imgui.NextColumn()
					imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.Always, imgui.ImVec2(0.5, 0.5))
					if imgui.CenterColumnSmallButton(fa.PEN_TO_SQUARE .. "##fraction") then
						imgui.StrCopy(MODULE.Main.input, u8(modules.player.data.fraction or ""))
					...
					if imgui.CenterColumnSmallButton(fa.PEN_TO_SQUARE .. "##rank") then
						imgui.StrCopy(MODULE.Main.input, u8(modules.player.data.fraction_rank or ""))
					...
					if imgui.BeginPopupModal(getHelperIcon() .. u8' Должность в организации ' .. getHelperIcon() .. '##fraction_rank', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar) then
						...
						if imgui.Button(fa.FLOPPY_DISK .. u8' Сохранить##save_fraction_rank', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
							...
							if old_rank_number < 9 and modules.player.data.fraction_rank_number >= 9 then
								sampAddChatMessage("[YouHelper] {FFFFFF}Поскольку вы стали " .. (modules.player.data.fraction_rank_number == 10 and 'лидером' or 'заместителем') .. ", нужно перезагрузить хелпер для пременения доп.функций. Перезагрузка...", message_color)
								reload_script = true
								thisScript():reload()
							end	
							imgui.CloseCurrentPopup()
					...
					imgui.NextColumn()
					imgui.CenterColumnText(u8(modules.player.data.fraction_tag or ""))
					imgui.NextColumn()
					if imgui.CenterColumnSmallButton(fa.PEN_TO_SQUARE .. '##fraction_tag') then
						imgui.StrCopy(MODULE.Main.input, u8(modules.player.data.fraction_tag or ""))
				...
				if imgui.BeginChild('##3', imgui.ImVec2(589 * settings.general.custom_dpi, 27 * settings.general.custom_dpi), true) then
					if thisScript().version:find('VIP') then
						...
						imgui.CenterText(u8(" VIP-пользователь " .. MODULE.Activate.user .. "[" .. MODULE.Activate.uid .. "]. Вам доступны все функции. "))
					else


						imgui.CenterColorText(imgui.ImVec4(0.5,0.8,1,1), u8"Обновления на сайте nizaev.org")


						if false then -- vip hidden
							imgui.OpenPopup(u8' YouHelper подписка на хелпер ')
						end
						imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.Always, imgui.ImVec2(0.5, 0.5))
						if imgui.BeginPopupModal(u8' YouHelper подписка на хелпер ', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar) then
							change_dpi()
							imgui.CenterUnderlineText(u8("https://youhelper.com/vip"))
							if imgui.IsItemHovered() then
								imgui.SetTooltip('https://youhelper.com/vip')
							end
							if imgui.IsItemClicked() then openLink('https://youhelper.com/vip') imgui.CloseCurrentPopup() end
							imgui.Separator()
							imgui.CenterText(u8'Либо свяжитесь с YouHelper:')
							if imgui.Button(u8('Telegram'), imgui.ImVec2(100 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
								openLink('https://t.me/youhelper/60')
								imgui.CloseCurrentPopup()
							end
							imgui.SameLine()
							if imgui.Button(u8('GitHub'), imgui.ImVec2(100 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
								openLink('https://github.com/rtwhrtt/youhelper')
								imgui.CloseCurrentPopup()
							end
				...
				if imgui.BeginTabBar('Список всех команд') then
					if imgui.BeginTabItem(fa.BARS..u8' Стандартные команды') then 
						if imgui.BeginChild('##standart_cmds', imgui.ImVec2(589 * settings.general.custom_dpi, 338 * settings.general.custom_dpi), true) then
							...
							imgui.Separator()
							imgui.Columns(2)
							imgui.CenterColumnText(u8"/mcruise")
							imgui.NextColumn()
							imgui.CenterColumnText(u8"Меню круиз-контроля (скорость, нитро)")
							imgui.Columns(1)
							imgui.Separator()
							if not isMode('none') then
								imgui.Columns(2)
						...
						function render_fastmenu(name, use, text, text2)
							if imgui.BeginChild('##fastmenu'..name, imgui.ImVec2(193.3 * settings.general.custom_dpi, 338 * settings.general.custom_dpi), true) then
								...
								if imgui.Button(fa.GEAR .. u8(' Настроить команды меню ') .. "##" .. name) then
									if name == 'Leader FastMenu' and modules.player.data.fraction_rank_number < 9 then
										sampAddChatMessage('[YouHelper] {ffffff}Данное лидерское фастменю доступно только для 9 или 10 ранга!', message_color)
									else
							...
							if IS_MOBILE then
								...
								if imgui.CenterButton(settings.general.piemenu and fa.TOGGLE_ON .. u8(' Отключить') or fa.TOGGLE_OFF .. u8(' Включить')) then
									if pie_no_errors then
										...
										sampAddChatMessage('[YouHelper] {ffffff}У вас отсуствует библиотека PieMenu, невозможно включить/настроить круговое меню!', message_color)
									end
								end
							end
							imgui.SetCursorPosY(120 * settings.general.custom_dpi)
								...
								if imgui.BeginPopupModal(fa.CIRCLE_PLUS .. u8' Выберите что именно нужно добавить ' .. fa.CIRCLE_PLUS, _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar) then
									...
									if imgui.ItemSelector(u8'', { u8'Один пункт', u8'Подменю для пунктов' }, MODULE.PieMenu.editor.selector, 200 * settings.general.custom_dpi) then
										...
										if number < 50 then
											...
											if thisScript().version:find('VIP') then
												sampAddChatMessage('[YouHelper] {ffffff}Для стабильности лимит 8 элементов в одном меню, используйте подменю!', message_color)
											else
												sampAddChatMessage('[YouHelper] {ffffff}Ограничение 5 (в VIP 8) элементов в одном меню, используйте подменю или VIP!', message_color)
											end
										end
										imgui.CloseCurrentPopup()
									end
						...
						if IS_MOBILE then
								...
								for index, button in ipairs(modules.buttons.data) do
									...
									if imgui.SmallButton((button.enable and fa.TOGGLE_ON or fa.TOGGLE_OFF) .. '##mb_toggle' .. index) then
										...
										if MODULE.Buttons[WindowName] then
											...
											sampAddChatMessage('[YouHelper] {ffffff}Кнопка заработает только после перезагрузки хелпера или перезахода в игру!', message_color)
											play_sound()
										end
										save_module('buttons')
									end
									...
									if imgui.BeginPopupModal(fa.PEN_TO_SQUARE .. u8' Редактирование кнопки ' .. fa.PEN_TO_SQUARE .. '##' .. index, _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar) then
										...
										if imgui.Button(fa.FLOPPY_DISK .. u8' Сохранить', imgui.ImVec2(100 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
											...
											save_module('buttons')
											sampAddChatMessage('[YouHelper] {ffffff}Размер изменится только после перезагрузки хелпера или перезахода в игру!', message_color)
											play_sound()
											imgui.CloseCurrentPopup()
										end
										imgui.EndPopup()
							...
							if imgui.Button(fa.CIRCLE_PLUS .. u8' Добавить кнопку##add_mobile_button', imgui.ImVec2(imgui.GetMiddleButtonX(1), 0)) then
								if #modules.buttons.data > 5 then
									sampAddChatMessage('[YouHelper] {ffffff}В бесплатной версии ограничение всего 5 кнопок! Купите VIP версию для безлимитных кнопок!', message_color)
									return
								end
			...
			if imgui.BeginTabItem(fa.FILE_PEN..u8' Заметки') then 
				...
				if imgui.Button(fa.CIRCLE_PLUS .. u8' Создать заметку', imgui.ImVec2(imgui.GetMiddleButtonX(1), 0)) then
					if #modules.notes.data > 100 then
						-- free
						return
					end
			...
			if imgui.BeginTabItem(fa.GEAR..u8' Настройки') then 
				if imgui.BeginChild('##1', imgui.ImVec2(589 * settings.general.custom_dpi, 187 * settings.general.custom_dpi), true) then
					...
					imgui.CenterText(u8('YouHelper v' .. thisScript().version))
					imgui.Text(fa.CIRCLE_USER..u8" Разработчик: Низаев Булат / Bulat_Nabiev")
					imgui.Separator()
					imgui.Text(fa.CIRCLE_INFO..u8" Версия хелпера: " .. u8(thisScript().version))
					imgui.Separator()
					imgui.Text(fa.BOOK ..u8" Руководство по использованию:")
					imgui.SameLine()
					if false then -- youtube removed
						openLink('https://nizaev.org')
					end
					if imgui.IsItemHovered() then
					...
					imgui.SameLine()
					if imgui.SmallButton(u8'nizaev.org') then
						openLink('https://nizaev.org')
					end
					if imgui.IsItemHovered() then
						imgui.SetTooltip(u8'Открыть nizaev.org')
					end
					...
					imgui.Separator()
					imgui.CenterText(u8"Остальное ПО и скрипты на сайте nizaev.org")
				...
				if imgui.BeginChild('##2', imgui.ImVec2(589 * settings.general.custom_dpi, 135 * settings.general.custom_dpi), true) then
					...
					if monet_no_errors then
						...
						if imgui.RadioButtonIntPtr(u8" Сustom ", MODULE.Main.theme, 0) then
							...
							sampAddChatMessage('[YouHelper] {ffffff}Установите библиотеку MoonMonet!', message_color)
						end
					end
					...
					imgui.PushItemWidth(138 * settings.general.custom_dpi)
					imgui.SetCursorPosY((72 * settings.general.custom_dpi))
					imgui.SliderFloat('##slider_helper_size', MODULE.Main.slider.dpi, 0.5, 2.0)
					...
					if imgui.BeginPopupModal(fa.TRIANGLE_EXCLAMATION .. u8' Предупреждение ' .. fa.TRIANGLE_EXCLAMATION .. '##change_size', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar) then
						...
						if imgui.Button(fa.CIRCLE_ARROW_RIGHT .. u8' Да, изменить##change_size', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
							...
							if IS_MOBILE and new_dpi < MONET_DPI_SCALE then
								sampAddChatMessage('[YouHelper] {ffffff}Для вашего дисплея нельзя сделать размер меньше ' .. MONET_DPI_SCALE, message_color)
								imgui.CloseCurrentPopup()
							else
								settings.general.custom_dpi = new_dpi
								save_settings()
								sampAddChatMessage('[YouHelper] {ffffff}Если интерфейс будет слишком ' .. size_text .. ', то используйте команду ' .. message_color_hex .. '/fixsize', message_color)
								sampAddChatMessage('[YouHelper] {ffffff}Перезагрузка скрипта для изменения размера интерфейса...', message_color)
								reload_script = true
								thisScript():reload()
							end
						end
				...
				if imgui.BeginChild("##3",imgui.ImVec2(589 * settings.general.custom_dpi, 35 * settings.general.custom_dpi),true) then
					if imgui.Button(fa.POWER_OFF .. u8" Отключить хелпер", imgui.ImVec2(imgui.GetMiddleButtonX(3), 25 * settings.general.custom_dpi)) then
						sampAddChatMessage('[YouHelper] {ffffff}Хелпер приостановил свою работу до следующего входа в игру.', message_color)
						if not IS_MOBILE then
							sampAddChatMessage('[YouHelper] {ffffff}Либо используйте сочетание клавиш ' .. message_color_hex .. 'CTRL {ffffff}+ ' .. message_color_hex .. 'R{ffffff}, чтобы повторно запустить хелпер.', message_color)
						end
						reload_script = true
						thisScript():unload()
					end
					...
					if imgui.BeginPopupModal(fa.TRIANGLE_EXCLAMATION .. u8' Предупреждение ' .. fa.TRIANGLE_EXCLAMATION .. '##delete_helper', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar) then
						change_dpi()
						imgui.CenterText(u8'Вы действительно хотите удалить YouHelper?')
			...
			if imgui.Button(fa.KEYBOARD .. u8' Забиндить##binder_bind', imgui.ImVec2(imgui.GetMiddleButtonX(IS_MOBILE and 4 or 5), 0)) then
				if MODULE.Binder.ComboTags[0] == 0 then
					if hotkey_no_errors then
						...
						sampAddChatMessage('[YouHelper] {ffffff}Данная функция недоступна, у вас отсуствуют файлы библиотеки mimgui_hotkeys!', message_color)
					end
				else
					sampAddChatMessage('[YouHelper] {ffffff}Данная функция доступа только если команда "Без аргументов"', message_color)
				end
			end
			imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.Always, imgui.ImVec2(0.5, 0.5))
			...
			if cmd:find("[^%w_]") or cmd == '' or desc == ''  or text_value == '' then
				...
				for _, command in ipairs(temp_array) do
					if command.cmd == MODULE.Binder.data.change_cmd and command.arg == MODULE.Binder.data.change_arg and command.text:gsub('&', '\n') == MODULE.Binder.data.change_text then
						...
						save_module('commands')
						if command.arg == '' then
							sampAddChatMessage('[YouHelper] {ffffff}Команда ' .. message_color_hex .. '/' .. new_command .. ' {ffffff}успешно сохранена!', message_color)
						elseif command.arg == '{arg}' then
							sampAddChatMessage('[YouHelper] {ffffff}Команда ' .. message_color_hex .. '/' .. new_command .. ' [аргумент] {ffffff}успешно сохранена!', message_color)
						elseif command.arg == '{id}' then
							sampAddChatMessage('[YouHelper] {ffffff}Команда ' .. message_color_hex .. '/' .. new_command .. ' [ID игрока] {ffffff}успешно сохранена!', message_color)
						elseif command.arg == '{id} {arg}' then
							sampAddChatMessage('[YouHelper] {ffffff}Команда ' .. message_color_hex .. '/' .. new_command .. ' [ID игрока] [аргумент] {ffffff}успешно сохранена!', message_color)
						elseif command.arg == '{id} {number} {arg}' then
							sampAddChatMessage('[YouHelper] {ffffff}Команда ' .. message_color_hex .. '/' .. new_command .. ' [ID игрока] [число] [аргумент] {ffffff}успешно сохранена!', message_color)
						end
...
function rp_spass_action(id)
	lua_thread.create(function()
		sampSendChat("/do Паспорт в кармане.")
		wait(1500)
		sampSendChat("/me достал(а) паспорт")
		wait(1500)
		sampSendChat("/do Паспорт в руке.")
		wait(1500)
		sampSendChat("/me передал(а) паспорт человеку напротив")
		wait(1500)
		sampSendChat("/showpass " .. id)
	end)
end

function rp_slic_action(id)
	lua_thread.create(function()
		sampSendChat("/do Папка с лицензиями в кармане.")
		wait(1500)
		sampSendChat("/me достал(а) папку с лицензиями")
		wait(1500)
		sampSendChat("/do Папка с лицензиями в руке.")
		wait(1500)
		sampSendChat("/me передал(а) папку с лицензиями человеку напротив")
		wait(1500)
		sampSendChat("/showlic " .. id)
	end)
end

function rp_smc_action(id)
	lua_thread.create(function()
		sampSendChat("/do Медкарта в кармане.")
		wait(1500)
		sampSendChat("/me достал(а) мед.карту")
		wait(1500)
		sampSendChat("/do Мед.карта в руке.")
		wait(1500)
		sampSendChat("/me передал(а) мед.карту человеку напротив")
		wait(1500)
		sampSendChat("/showmc " .. id)
	end)
end

function rp_sskill_action(id)
	lua_thread.create(function()
		sampSendChat("/do Выписка из тира в кармане.")
		wait(1500)
		sampSendChat("/me достал(а) выписку из тира")
		wait(1500)
		sampSendChat("/do Выписка из тира в руке.")
		wait(1500)
		sampSendChat("/me показал(а) выписку из тира человеку напротив")
		wait(1500)
		sampSendChat("/showskill " .. id)
	end)
end
-- ====== MCRUISE ======
MODULE.MCruise = { Window = imgui.new.bool(false) }
local mcruise_input_speed = imgui.new.int(80)

imgui.OnFrame(
    function() return MODULE.MCruise.Window[0] end,
    function(player)
        imgui.SetNextWindowPos(imgui.ImVec2(sizeX/2, sizeY/2), imgui.Cond.Always, imgui.ImVec2(0.5,0.5))
        imgui.SetNextWindowSize(imgui.ImVec2(300 * settings.general.custom_dpi, 0), imgui.Cond.Always)
        if imgui.Begin(u8"/mcruise ##mcruisewnd", MODULE.MCruise.Window, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize) then
            local sc = settings.general.mcruise_enable and imgui.ImVec4(0.2,0.9,0.4,1) or imgui.ImVec4(0.9,0.3,0.3,1)
            local st = settings.general.mcruise_enable and u8"Автопилот включён" or u8"Введите /cruise и поставьте метку на карте"
            imgui.CenterColorText(sc, st)
            imgui.Separator()
            imgui.Text(u8"Максимальная скорость (km/h):")
            mcruise_input_speed[0] = settings.general.mcruise_speed
            if imgui.SliderInt("##mcs", mcruise_input_speed, 1, 500) then
                settings.general.mcruise_speed = mcruise_input_speed[0]
                save_settings()
            end
            imgui.Separator()
            local nitro = imgui.new.bool(settings.general.mcruise_nitro)
            if imgui.Checkbox(u8"Автонитро", nitro) then
                settings.general.mcruise_nitro = nitro[0]; save_settings()
            end
            imgui.Separator()
            if imgui.Button(u8"Сбросить по умолчанию##mcreset", imgui.ImVec2(-1,0)) then
                settings.general.mcruise_speed = 60
                settings.general.mcruise_nitro = false
                save_settings()
            end
        end
        imgui.End()
...
function firs_render_assist_gui()
	...
	render_assist_item(
		"RP отыгровка показа документов",
		"Автоматически выполняет RP отыгровку перед показом документов.",
		settings.general,
		"rp_show_docs"
	)
	render_assist_item(
		"Пинг в чате @" .. (MODULE.Binder.tag.my_nick() or ""),
		...
		if imgui.BeginTabBar('FractinFunctions') then
			...
			if imgui.BeginTabItem(fa.STAR .. u8' Система умного розыска') then 
				renderSmartGUI(
					...
					fa.STAR,
					'https://raw.githubusercontent.com/rtwhrtt/youhelper/main/SmartUK/' .. getServerNumber() .. '/SmartUK.json',
			...
			if imgui.BeginTabItem(fa.TICKET .. u8' Система умных штрафов') then 
				renderSmartGUI(
					...
					fa.TICKET, 
					'https://raw.githubusercontent.com/rtwhrtt/youhelper/main/SmartPDD/' .. getServerNumber() .. '/SmartPDD.json', 
		...
		if imgui.BeginTabBar('FractinFunctions') then
			...
			if imgui.BeginTabItem(fa.STAR .. u8' Система умного продления срока') then 
				renderSmartGUI(
					...
					fa.TICKET, 
					'https://raw.githubusercontent.com/rtwhrtt/youhelper/main/SmartRPTP/' .. getServerNumber() .. '/SmartRPTP.json', 
			...
			if imgui.BeginTabItem(fa.ROBOT .. u8' Личный помощник "Ассистент"') then 
				if imgui.BeginChild('##smi_assist', imgui.ImVec2(589 * settings.general.custom_dpi, 338 * settings.general.custom_dpi), true) then	
					...
					render_assist_item(
						...
						"Генерация редактирования объявлений доступна в хелпере!",
									...
									if imgui.Button(fa.TRASH_CAN .. u8' Удалить', imgui.ImVec2(imgui.GetMiddleButtonX(3), 25 * settings.general.custom_dpi)) then
										for id, ad in ipairs(modules.ads_history.data) do
											if ad.text == MODULE.SmiEdit.adshistory_orig then
												...
												save_module('ads_history')
												sampAddChatMessage("[YouHelper] {ffffff}Обьявление из истории успешно удалено!", message_color)
												break
											end
										end
										imgui.CloseCurrentPopup()
									...
									if imgui.Button(fa.FLOPPY_DISK .. u8' Сохранить', imgui.ImVec2(imgui.GetMiddleButtonX(3), 25 * settings.general.custom_dpi)) then
										for id, ad in ipairs(modules.ads_history.data) do
											if ad.text == MODULE.SmiEdit.adshistory_orig then
												...
												save_module('ads_history')
												sampAddChatMessage("[YouHelper] {ffffff}Обьявление из истории успешно изменено и сохранено!", message_color)
												break
											end
										end
										imgui.CloseCurrentPopup()
...
if (not isMode('none')) then
	imgui.OnFrame(
		...
		function(player)
			if #MODULE.Members.all == 0 then
				sampAddChatMessage('[YouHelper] {ffffff}Ошибка, список сотрудников пустой!', message_color)
				MODULE.Members.Window[0] = false
	...
	imgui.OnFrame(
		...
		function(player)
			imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
			imgui.Begin(getHelperIcon().." YouHelper " .. getHelperIcon() .. "##rank", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize)
			change_dpi()
...
if not (isMode('ghetto') or isMode('mafia')) then
			...
			if MODULE.Sobes.player_id ~= nil and isParamSampID(MODULE.Sobes.player_id) then
				...
				if imgui.BeginChild('sobes1', imgui.ImVec2(240 * settings.general.custom_dpi, 180 * settings.general.custom_dpi), true) then
					...
					if imgui.Button(fa.PLAY .. u8" Начать собеседование", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
						lua_thread.create(function()
							sampSendChat("Здравствуйте, я " .. (modules.player.data.name_surname or "?") .. " - " .. modules.player.data.fraction_rank .. ' ' .. modules.player.data.fraction_tag)
							wait(1500)
				...
				if imgui.BeginChild('sobes2', imgui.ImVec2(240 * settings.general.custom_dpi, 180 * settings.general.custom_dpi), true) then
					...
					imgui.Separator()
					if imgui.Button(fa.GLOBE .. u8" Наличие спец.рации", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
						sampSendChat("Имеется ли у Вас спец. рация?")
					end
				...
				sampAddChatMessage('[YouHelper] {ffffff}Прозиошла ошибка, ID игрока недействителен!', message_color)
				MODULE.Sobes.Window[0] = false
			end
		end
	)
	...
	imgui.OnFrame(
		...
		function(player)
			...
			imgui.Begin(getHelperIcon() .. u8" YouHelper " .. getHelperIcon() .. '##post_info_menu', MODULE.Post.Window, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize )
			change_dpi()
			...
			if MODULE.Post.active then
				...
				if imgui.SmallButton(fa.GEAR) then
					imgui.OpenPopup(fa.BUILDING_SHIELD .. u8(' YouHelper##post_select_code'))
				end
				imgui.Separator()
			...
			if imgui.BeginPopup(fa.BUILDING_SHIELD .. u8(' YouHelper##post_select_code'), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  ) then
				change_dpi()
				player.HideCursor = false 
...
if isMode('police') or isMode('fbi') or isMode('prison') then
	...
	function renderSmartGUI(title, icon, downloadPath, editPopupTitle, data, saveFunction, usageText, pathDisplay, download_file_name, download_item)
		if imgui.BeginChild('##smart'..title, imgui.ImVec2(589 * settings.general.custom_dpi, 338 * settings.general.custom_dpi), true, imgui.WindowFlags.NoScrollbar) then
			...
			imgui.CenterText(u8'Сообщите SMART-редакторам на нашем GitHub-странице.')
			imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.Always, imgui.ImVec2(0.5, 0.5))
											...
											if imgui.BeginPopupModal(u8("Редактирование подпункта##") .. title .. chapter.name .. index .. chapter_index, _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar) then
												...
												if imgui.Button(fa.FLOPPY_DISK .. u8' Сохранить##saveedititem', imgui.ImVec2(imgui.GetMiddleButtonX(2), 0)) then
													...
													if isValid then
														...
														sampAddChatMessage('[YouHelper] {ffffff}Ошибка в указанных данных, исправьте!', message_color)
													end
												end
												imgui.EndPopup()
											end
	...
	imgui.OnFrame(
		...
		function(player)
			...
			if modules.smart_rptp.data ~= nil and isParamSampID(MODULE.PumMenu.player_id) then
				...
				sampAddChatMessage('[YouHelper] {ffffff}Произошла ошибка умного срока (нету данных либо игрок офнулся)!', message_color)
...
if isMode('police') or isMode('fbi') then
	imgui.OnFrame(
		...
		function(player)
			imgui.SetNextWindowPos(imgui.ImVec2(settings.windows_pos.patrool_menu.x, settings.windows_pos.patrool_menu.y), imgui.Cond.FirstUseEver)
			imgui.Begin(getHelperIcon() .. u8" YouHelper " .. getHelperIcon() .. '##patrool_info_menu', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize )
			change_dpi()
			safery_disable_cursor(player)
			if MODULE.Patrool.active then
				...
				imgui.SameLine()
				if imgui.SmallButton(fa.GEAR) then
					imgui.OpenPopup(fa.BUILDING_SHIELD .. u8(' YouHelper##patrool_select_code'))
				end
				imgui.Separator()
			...
			if imgui.BeginPopup(fa.BUILDING_SHIELD .. u8(' YouHelper##patrool_select_code'), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  ) then
				change_dpi()
				player.HideCursor = false 
				imgui.PushItemWidth(150 * settings.general.custom_dpi)
				if imgui.Combo('##patrool_code', MODULE.Patrool.ComboCode, MODULE.Patrool.ImItemsCode, #MODULE.Patrool.codes) then
	...
	imgui.OnFrame(
		...
		function(player)
			...
			if tonumber(#MODULE.Wanted.all) == 0 then 
				sampAddChatMessage('[YouHelper] {ffffff}Сейчас на сервере нету игроков с розыском!', message_color)
				MODULE.Wanted.Window[0] = false
			end
			...
			if settings.mj.auto_update_wanteds then
				...
				if imgui.Button(u8'Обновить список преступников', imgui.ImVec2(340 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
					MODULE.Wanted.Window[0] = false
					sampAddChatMessage('[YouHelper] {ffffff}Вы можете включить авто-обновление списка /wanteds в /helper - Функции ' .. modules.player.data.fraction_tag .. '!', message_color)
	...
	imgui.OnFrame(
		...
		function(player)
			...
			if modules.smart_uk.data ~= nil and isParamSampID(MODULE.SumMenu.player_id) then
				...
				sampAddChatMessage('[YouHelper] {ffffff}Произошла ошибка умного розыска (нету данных либо игрок офнулся)!', message_color)
				MODULE.SumMenu.Window[0] = false
			end
			imgui.End()
		end
	...
	imgui.OnFrame(
		...
		function(player)
			...
			if modules.smart_pdd.data ~= nil and isParamSampID(MODULE.TsmMenu.player_id) then
				...
				sampAddChatMessage('[YouHelper] {ffffff}Произошла ошибка умных штрафов (нету данных либо игрок офнулся)!', message_color)
				MODULE.TsmMenu.Window[0] = false
			end
			imgui.End()
		end
...
if isMode('hospital') then
	imgui.OnFrame(
		...
		function(player)
			imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
			imgui.Begin(fa.HOSPITAL.." YouHelper " .. fa.HOSPITAL .. "##medcard", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize)
			change_dpi()
	...
	imgui.OnFrame(
		...
		function(player)
			imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
			imgui.Begin(fa.HOSPITAL.." YouHelper " .. fa.HOSPITAL .. "##recept", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize)
			change_dpi()
	...
	imgui.OnFrame(
		...
		function(player)
			imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
			imgui.Begin(fa.HOSPITAL.." YouHelper " .. fa.HOSPITAL .. "##ant", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize)
			change_dpi()
	...
	imgui.OnFrame(
		...
		function(player)
			...
			imgui.Begin(fa.HOSPITAL.." YouHelper " .. fa.HOSPITAL .. "##fast_heal", MODULE.HealChat.Window, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoTitleBar +  imgui.WindowFlags.AlwaysAutoResize )
			change_dpi()
...
if isMode('smi') then
	imgui.OnFrame(
		...
		function(player)
			...
			imgui.Begin(getHelperIcon() .. u8" YouHelper " .. getHelperIcon() .. '##MODULE.SmiEdit.Window', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar )
			change_dpi()
			...
			if imgui.Button(fa.CIRCLE_ARROW_RIGHT .. u8(send_ad_label), imgui.ImVec2(imgui.GetMiddleButtonX(3), 25 * settings.general.custom_dpi)) then
				...
				if MODULE.SmiEdit.vip_pause then
					lua_thread.create(function()
						sampAddChatMessage('[YouHelper | Ассистент] {ffffff}Серверное КД 10 сек после VIP обьявы, ждите...', message_color)
						play_sound()
		...
		function(player)
			...
			if settings.gov.auto_update_zeks then
				...
				if imgui.Button(u8'Обновить список заключенных', imgui.ImVec2(450 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
					MODULE.Zeks.Window[0] = false
					sampAddChatMessage('[YouHelper] {ffffff}Вы можете включить авто-обновление списка /zeks в /helper - Функции ' .. modules.player.data.fraction_tag .. '!', message_color)
...
imgui.OnFrame(
    ...
    function(player)
		...
		if not check then
			sampAddChatMessage('[YouHelper] {ffffff}Настройте FastMenu в /helper - Команды и RP отыгровки - Фаст Меню - FastMenu', message_color)
			MODULE.FastMenu.Window[0] = false
		end
		imgui.End()
    end
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(fa.BUILDING_SHIELD .." YouHelper##fast_menu_button", MODULE.FastMenuButton.Window, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoBackground + imgui.WindowFlags.NoScrollbar)
		change_dpi()
...
imgui.OnFrame(
    ...
    function(player)
		...
		if IS_MOBILE and not check then
			sampAddChatMessage('[YouHelper] {ffffff}Настройте Leader FastMenu в /helper - Команды и RP отыгровки - Фаст Меню - Leader FastMenu', message_color)
			MODULE.FastMenu.Window[0] = false
...
----------------------------------------- PLAYER MENU (E) -------------------------------------------
MODULE.PlayerMenu = { Window = imgui.new.bool(false), player_id = 0 }

imgui.OnFrame(
	function() return MODULE.PlayerMenu.Window[0] end,
	function(player)
		local pid = MODULE.PlayerMenu.player_id
		local nick = sampGetPlayerNickname(pid) or ""
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX/2, sizeY/2), imgui.Cond.Always, imgui.ImVec2(0.5,0.5))
		imgui.SetNextWindowSize(imgui.ImVec2(300 * settings.general.custom_dpi, 0), imgui.Cond.Always)
		if imgui.Begin(u8("Меню игсрока: ") .. nick .. "##playermenu", MODULE.PlayerMenu.Window, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize) then
			if settings.general.rp_spass then
				if imgui.Button(u8"Руководство по использованию: Росставить паспорт /spass", imgui.ImVec2(-1, 0)) then
					rp_spass_action(pid)
					MODULE.PlayerMenu.Window[0] = false
				end
			end
			if settings.general.rp_slic then
				if imgui.Button(u8"Руководство по использованию: Росставить лицензии /slic", imgui.ImVec2(-1, 0)) then
					rp_slic_action(pid)
					MODULE.PlayerMenu.Window[0] = false
				end
			end
			if settings.general.rp_smc then
				if imgui.Button(u8"Руководство по использованию: Росставить мед.карту /smc", imgui.ImVec2(-1, 0)) then
					rp_smc_action(pid)
					MODULE.PlayerMenu.Window[0] = false
				end
			end
			if settings.general.rp_sskill then
				if imgui.Button(u8"Руководство по использованию: Росставить навыки /sskill", imgui.ImVec2(-1, 0)) then
					rp_sskill_action(pid)
					MODULE.PlayerMenu.Window[0] = false
				end
			end
			if imgui.Button(u8"Разборные команды /hm", imgui.ImVec2(-1, 0)) then
				sampSetChatInputEnabled(true)
				sampSetChatInputText("/hm " .. pid .. " ")
				MODULE.PlayerMenu.Window[0] = false
			end
			if imgui.Button(u8"Закховат /su", imgui.ImVec2(-1, 0)) then
				sampSendChat("/su " .. pid)
				MODULE.PlayerMenu.Window[0] = false
			end
	...
	function(player)
		...
		if pie.BeginPiePopup('PieMenu', 2) then
			...
			if #modules.piemenu.data == 0 then
				sampAddChatMessage('[YouHelper] {ffffff}Настройте или отключите PieMenu в /helper - Команды и RP отыгровки - Фаст Меню - PieMenu', message_color)
			end
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.Always, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.CIRCLE_INFO .. u8" Доступно обновление YouHelper ".. fa.CIRCLE_INFO .. "##update_window", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize )
		...
		if not MODULE.Update.version:find('VIP') then
			...
			if imgui.Button(fa.DOWNLOAD ..u8' Обновить до ' .. u8(MODULE.Update.version), imgui.ImVec2(250 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
				...
				downloadFileFromUrlToPath(MODULE.Update.url, worked_dir .. "/YouHelper.lua")
				MODULE.Update.Window[0] = false
			end
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(getHelperIcon() .. " YouHelper " .. getHelperIcon() .. "##MODULE.CommandStop.Window", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize)
		change_dpi()
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY - 50 * settings.general.custom_dpi), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(getHelperIcon() .." YouHelper " .. getHelperIcon() .. "##MODULE.CommandPause.Window", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize)
		change_dpi()
...
function apply_dark_theme()
	pcall(function() imgui.SwitchContext() end)
	local d=settings.general.custom_dpi or 1;local s=imgui.GetStyle()
	s.WindowPadding=imgui.ImVec2(10*d,10*d);s.FramePadding=imgui.ImVec2(7*d,4*d)
	s.ItemSpacing=imgui.ImVec2(7*d,6*d);s.ItemInnerSpacing=imgui.ImVec2(5*d,4*d)
	s.ScrollbarSize=7*d;s.GrabMinSize=10*d;s.WindowBorderSize=1*d;s.ChildBorderSize=1*d
	s.PopupBorderSize=1*d;s.FrameBorderSize=0;s.TabBorderSize=0
	s.WindowRounding=14*d;s.ChildRounding=8*d;s.FrameRounding=7*d;s.PopupRounding=12*d
	s.ScrollbarRounding=7*d;s.GrabRounding=5*d;s.TabRounding=7*d
	s.WindowTitleAlign=imgui.ImVec2(0.5,0.5);s.ButtonTextAlign=imgui.ImVec2(0.5,0.5)
	s.SelectableTextAlign=imgui.ImVec2(0.5,0.5)
	local C=s.Colors;local v=imgui.ImVec4
	C[imgui.Col.Text]=v(0.95,0.95,0.99,1);C[imgui.Col.TextDisabled]=v(0.40,0.38,0.52,1)
	C[imgui.Col.WindowBg]=v(0.05,0.04,0.09,1);C[imgui.Col.ChildBg]=v(0.07,0.06,0.12,1)
	C[imgui.Col.PopupBg]=v(0.07,0.06,0.12,0.98);C[imgui.Col.Border]=v(0.55,0.15,0.90,0.40)
	C[imgui.Col.BorderShadow]=v(0,0,0,0);C[imgui.Col.FrameBg]=v(0.10,0.08,0.18,1)
	C[imgui.Col.FrameBgHovered]=v(0.20,0.12,0.35,1);C[imgui.Col.FrameBgActive]=v(0.28,0.18,0.50,1)
	C[imgui.Col.TitleBg]=v(0.08,0.05,0.16,1);C[imgui.Col.TitleBgActive]=v(0.45,0.08,0.85,1)
	C[imgui.Col.TitleBgCollapsed]=v(0.08,0.05,0.16,0.80);C[imgui.Col.MenuBarBg]=v(0.08,0.05,0.16,1)
	C[imgui.Col.ScrollbarBg]=v(0.05,0.04,0.09,1);C[imgui.Col.ScrollbarGrab]=v(0.50,0.10,0.90,0.70)
	C[imgui.Col.ScrollbarGrabHovered]=v(0.60,0.18,1,0.88);C[imgui.Col.ScrollbarGrabActive]=v(0.70,0.25,1,1)
	C[imgui.Col.CheckMark]=v(0.78,0.30,1,1);C[imgui.Col.SliderGrab]=v(0.58,0.12,0.98,0.90)
	C[imgui.Col.SliderGrabActive]=v(0.70,0.25,1,1);C[imgui.Col.Button]=v(0.40,0.05,0.78,0.88)
	C[imgui.Col.ButtonHovered]=v(0.55,0.12,0.92,1);C[imgui.Col.ButtonActive]=v(0.65,0.20,1,1)
	C[imgui.Col.Header]=v(0.40,0.05,0.78,0.65);C[imgui.Col.HeaderHovered]=v(0.52,0.12,0.90,0.85)
	C[imgui.Col.HeaderActive]=v(0.62,0.20,1,1);C[imgui.Col.Separator]=v(0.45,0.08,0.85,0.45)
	C[imgui.Col.SeparatorHovered]=v(0.60,0.18,1,0.75);C[imgui.Col.SeparatorActive]=v(0.70,0.25,1,1)
	C[imgui.Col.ResizeGrip]=v(0.58,0.12,0.98,0.28);C[imgui.Col.ResizeGripHovered]=v(0.58,0.12,0.98,0.70)
	C[imgui.Col.ResizeGripActive]=v(0.70,0.25,1,0.95);C[imgui.Col.Tab]=v(0.15,0.08,0.30,1)
	C[imgui.Col.TabHovered]=v(0.52,0.12,0.90,1);C[imgui.Col.TabActive]=v(0.40,0.08,0.80,1)
	C[imgui.Col.TabUnfocused]=v(0.08,0.05,0.16,0.97);C[imgui.Col.TabUnfocusedActive]=v(0.18,0.08,0.36,1)
	C[imgui.Col.TextSelectedBg]=v(0.58,0.12,0.98,0.38);C[imgui.Col.DragDropTarget]=v(0.78,0.30,1,0.92)
	C[imgui.Col.NavHighlight]=v(0.58,0.12,0.98,1);C[imgui.Col.NavWindowingHighlight]=v(1,1,1,0.70)
	C[imgui.Col.NavWindowingDimBg]=v(0.18,0.08,0.38,0.22);C[imgui.Col.ModalWindowDimBg]=v(0.05,0.02,0.12,0.90)
end
...
function insert_to_cursor(insert_text, buffer)
    ...
    if MODULE.INPUT.USER_MOVED_CURSOR then
		...
		sampAddChatMessage('[YouHelper] {ffffff}Курсор для вставки установлен в конец строчки!', message_color)
    else
        start = #current
        finish = #current
    end
...
function sendAnalytics()
	asyncHttpRequest(
		"POST",
        "https://api.youhelper.com/v1/usage/launch",
        {
            headers = {
...
-------------------------------------------- Terminate ------------------------------------------
-- YouHelper commands
sampRegisterChatCommand("yh",function() MODULE.Main.Window[0]=not MODULE.Main.Window[0] end)
sampRegisterChatCommand("yhreset",function()
    if sizeX and sizeX>0 then
        settings.windows_pos={patrool_menu={x=sizeX/2,y=sizeY/2},post_menu={x=sizeX/2,y=sizeY/2},
            wanteds_menu={x=sizeX/2,y=sizeY/2},zeks_menu={x=sizeX*0.83,y=sizeY/2},
            mobile_fastmenu_button={x=sizeX*0.12,y=sizeY*0.43},pie={x=sizeX*0.7,y=sizeY*0.7}}
        save_settings();sampAddChatMessage("[YouHelper] {ffffff}Позиции сброшены!",message_color)
    end
end)
sampRegisterChatCommand("yhinfo",function()
    sampAddChatMessage("[YouHelper] {ffffff}=== YouHelper v"..thisScript().version.." ===",message_color)
    sampAddChatMessage("[YouHelper] {ffffff}GitHub: "..message_color_hex.."github.com/rtwhrtt/youhelper",message_color)
    sampAddChatMessage("[YouHelper] {ffffff}Telegram: "..message_color_hex.."@youhelper_rp2",message_color)
end)
local _cp_on,_cp_t=false,0
sampRegisterChatCommand("checkpoint",function()
    if not _cp_on then _cp_on=true;_cp_t=os.time()
        sampAddChatMessage("[YouHelper] {ffffff}Обход начался.",message_color)
    else _cp_on=false;local e=os.difftime(os.time(),_cp_t);local m=math.floor(e/60);local sc=e%60
        sampAddChatMessage("[YouHelper] {ffffff}Обход: "..m.." мн. "..sc.." с.",message_color)
        sampSendChat("/me завершил патрульный обход за "..m.." мн. "..sc.." с.")
    end
end)
...
function onScriptTerminate(script, game_quit)
    if script == thisScript() and not game_quit and not reload_script then
		...
		sampAddChatMessage('[YouHelper] {ffffff}Произошла неизвестная ошибка, хелпер приостановил свою работу!', message_color)
		if not IS_MOBILE then 
			sampAddChatMessage('[YouHelper] {ffffff}Используйте ' .. message_color_hex .. 'CTRL {ffffff}+ ' .. message_color_hex .. 'R {ffffff}чтобы перезапустить хелпер.', message_color)
		end
    end
end